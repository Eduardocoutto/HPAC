...
ID_cida INT,
nome VARCHAR(100),
FOREIGN KEY(ID_cida) REFERENCES estado (ID_cida) MATCH FULL ON DELETE RESTRICT ON UPDATE CASCADE
);
...
FOREIGN KEY(ID_bair) REFERENCES cidade (ID_bair) MATCH FULL ON DELETE RESTRICT ON UPDATE CASCADE
);
...
FOREIGN KEY (ID_tcon) REFERENCES tipo_contato (ID_tcon) MATCH FULL ON DELETE RESTRICT UPDATE CASCADE
);
